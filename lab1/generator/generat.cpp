...
#include <fstream>
 ...
 std::string Generator::generateString(std::ostream &os) {
     ...
     std::string res=generateSubstr(os, curLen/10);
     curLen -=curLen/10;

     while ((charDis(_generator) < curLen-numOpt*8) && curLen>0) {
         res.append(" -");
...
void Generator::genFile() {
     std::ofstream out;
     std::uniform_int_distribution<> charDis(1,6);
     ...
     for (int i=0; i<_numberOfString; i++) {
         ...
         out << generateString(out) << std::endl;
...
std::string Generator::mutateString(std::string str) {
     ...
     switch (a) {
         case 1:
             std::uniform_int_distribution<> DelLength(2,100);
            if (str.length()>10) {
                str.erase(str.begin() + DelLength(_generator), str.begin() +DelLength(_generator)+5);
            }
             break;
         case 2:
             str.insert(LenStr(_generator), "-", 2); break;
         case 3:
             std::shuffle(str.begin(), str.end(), _generator); break;
             default: break;
     }
     return str;
 }
